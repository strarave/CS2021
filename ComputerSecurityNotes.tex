	...
	\clearpage \part{Exercises tips and notes}
				...
				\subsubsection{Remainder: Exploiting Format String Bug in a ANSI C Program}
					...
					\paragraph{Writing Multiple Locations}
						...
						\begin{equation}
							...
							\end{cases}
						\end{equation}
						TA and TA+1 are written in the hexadecimal format with the "\textbackslash x" every byte AND remember that the bytes are to be written in reverse order (usually little endian).
						\begin{equation}
		...
		\section{Web Vulnerabilities}
			...
			\subsection{SQL Injection}
				...
				\end{itemize}
				This kind of attacks usually require to extract some information from a DB; an SQLi injection exploit follows a rigid structue
				\begin{enumerate}
					\item a string terminator (apice or double apices) that is used to cloe the parameter that can be exploited
