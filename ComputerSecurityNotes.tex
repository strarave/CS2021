...
\begin{document}
	...
	\clearpage \part{Exercises tips and notes}
		\section{Binary Exploits}
			...
			\subsection{Format String}
				\subsubsection{Remainder: Exploiting Format String Bug in a ANSI C Program}
					We have several placeholders and placeholders extensions we can use to make a format string vulnerability super dangerous.\\
					Placeholders:
					\begin{enumerate}
						\item \%d and \%i to print integers
					...
					\paragraph{Format String in Practice}
						...
						\begin{itemize}
							...
							\item \emph{An unchecked printing function that does not format the output}, or that uses the \%s format placeholder.
						\end{itemize}
					...
					\paragraph{Writing Multiple Locations}
						...
						\begin{equation}
							\begin{cases}
								\text{target addresses } TA, \,TA+2\\
								\text{malicious data } MD\\
								\text{offset of the target address } OFF, \,OFF+1\\
							\end{cases}
						\end{equation}
						TA and TA+2 are written in the hexadecimal format with the "\textbackslash x" every byte AND remember that the bytes are to be written in reverse order (usually little endian).
						REMINDER: watch out for the \emph{actual} written file from the print function. They're always $targetAddressBytes \,+\, charactersAlreadyPrintedByPrintf$
						\begin{equation}
							(TA) (TA+2) \,\%(low\vert\,MD\,\vert - chars\,printed)\, c\%(OFF)\$hn\%(high - low)\vert\,MD\vert\,c\%(OFF+1)\$hn
						\end{equation}
						in the case of $\vert first\, part \vert \,>\, \vert second\,part\vert $
						\begin{equation}
							(TA+2) (TA) \,\%(low\vert\,MD\,\vert - chars\,printed)\, c\%(OFF)\$hn\%(high - low)\vert\,MD\vert\,c\%(OFF+1)\$hn
						\end{equation}
						in the other case.
						A valid exploit to write "0x44674234" at address "0x42414515" is\\
						"\textbackslash x15\textbackslash x45\textbackslash x41\textbackslash x42\textbackslash x17\textbackslash x45\textbackslash x41\textbackslash x42\%16940c\%7\$hn\%563c\%8\$hn"
						...
						\subparagraph{Quick Note on this Part}
							...
							\begin{figure}[H]
								\centering
								\includegraphics[width = \textwidth]{./images/formatStringRecap1.png}
							\end{figure}
							\begin{figure}[H]
								\centering
								\includegraphics[width = \textwidth]{./images/formatStringRecap2.png} 
							\end{figure}
